...
function rest_calculator_register_routes() {
	register_rest_route( 'calc', 'add/(?P<o1>[-\.\d]+)/(?P<o2>[-\.\d]+)', array(
		'methods'  => 'GET',
		'callback' => array( new Calculator, 'add' ),
	) );

	register_rest_route( 'calc', 'times/(?P<o1>[-\.\d]+)/(?P<o2>[-\.\d]+)', array(
		'methods'  => 'GET',
		'callback' => array( new Calculator, 'times' ),
	) );

	register_rest_route( 'calc', 'hash/(?P<input>.*)', array(
		'methods'  => 'GET',
		'callback' => array( new Calculator, 'hash' ),
	) );
}

if ( function_exists( 'add_action' ) ) {
	add_action( 'rest_api_init', 'rest_calculator_register_routes' );
}
